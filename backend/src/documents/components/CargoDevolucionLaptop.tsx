    ...
    >
      ...
      <div className="relative flex h-full flex-col" style={{ zIndex: 10, isolation: "isolate" }}>
        ...
        <p className="leading-relaxed text-justify uppercase text-[11pt]">
          Recibí de <span className="font-bold">{empleado.nombre}</span>, el equipo laptop asignado con las siguientes características:
        </p>

        <table className="relative w-full mt-5 border-collapse text-[11pt]"
          style={{ zIndex: 2, backgroundColor: "transparent", border: "1px solid #000" }}>
          <tbody>
            {rows.map((row) => (
              <tr key={row.label} style={{ backgroundColor: "transparent" }}>
                <td className="px-2 py-1.5 font-normal w-[32%] uppercase align-top"
                  style={{ backgroundColor: "transparent", border: "1px solid #000", position: "relative", zIndex: 2 }}>{row.label}</td>
                <td className="px-2 py-1.5 font-bold uppercase align-top"
                  style={{ backgroundColor: "transparent", border: "1px solid #000", position: "relative", zIndex: 2 }}>{row.value}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="mt-12 flex justify-between gap-8">
          ...
          <div id="signature-block" className="w-[58mm] text-center relative">
            <div id="signature-field" data-field="signature"
