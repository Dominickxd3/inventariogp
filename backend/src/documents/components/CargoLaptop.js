...
export default function CargoLaptop({
  ...
  return h('div', {
    ...
    h('div', { 'aria-hidden': true, className: 'pointer-events-none absolute inset-0 z-0 flex items-center justify-center overflow-hidden', style: { zIndex: 0 } },
    ...
    h('div', { className: 'relative flex h-full flex-col', style: { zIndex: 1, isolation: 'isolate' } },
      ...
      h('h1', { className: 'text-center font-bold underline uppercase text-[12pt] tracking-wide mb-6' }, 'Cargo de entrega de equipo laptop'),
      h('p', { className: 'leading-relaxed text-justify uppercase text-[11pt]' },
        'Recibí de ', h('span', { className: 'font-bold' }, empresa.nombre), ' un equipo laptop con las siguientes características:'),
      ...
      h('div', { id: 'signature-block', className: 'mt-14 w-[58mm] text-center relative' },
        h('div', { id: 'signature-field', 'data-field': 'signature', className: 'relative mx-auto flex h-[18mm] w-full items-end justify-center' },
          firmaSrc
            ? h('img', { id: 'signature-image', src: firmaSrc, alt: 'Firma', className: 'max-h-[16mm] max-w-full object-contain' })
            : h('img', { id: 'signature-image', alt: '', className: 'hidden max-h-[16mm] max-w-full object-contain' }),
        ),
